 ...
 * and Contact. Tone controls the left-copy colors for dark vs. light
 * section backgrounds.
 */
export function FormCTA({
  ...
  const linkColor = tone === 'dark' ? 'text-orange' : 'text-orange-ink'
      ...
      <div>
        ...
        {href && phone && (
          <p className={`mt-5 font-inter ${bodyColor}`}>
            {callLead}{' '}
            <a href={href} className={`font-mono font-medium ${linkColor} hover:underline`}>
              {formatPhone(phone)}
            </a>
          </p>
        )}
